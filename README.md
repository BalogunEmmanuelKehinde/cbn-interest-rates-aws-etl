...
<img width="740" height="413" alt="dashboard" src="https://github.com/user-attachments/assets/d4fab8dd-7129-441a-9f08-34982821ce2a" />
...
| Layer | Technology |
|---|---|
| Data Extraction | Python, pdfplumber |
...
## Dashboard Pages
1. **Overview** — KPI cards, sector lending rates, savings rate by bank, rate trend over time
2. **Lending Rates Analysis** — Prime rate by sector, max rate by sector, prime vs max comparison
3. **Deposit Rates Analysis** — Savings, demand, and time deposit rates by bank
4. **Bank Spotlight** — Best bank per sector (lowest prime rate) — interactive matrix
5. **Rate Trends** — Prime, savings, and max rate movement across Q1 2026

## Project Structure
```
cbn-interest-rates-aws-etl/
├── scripts/
│   └── batch_convert.py
├── data/
│   └── processed/
...
```
3. Place CBN PDF files in `data/raw/` named as `cbn_rates_YYYY-MM-DD.pdf`
4. Run the batch converter:
```
python scripts/batch_convert.py
```
5. Upload CSVs to S3 and trigger the Glue job
6. Query via Athena or connect Power BI via ODBC

## Author
**Emmanuel Balogun** | Data Analyst  
[GitHub](https://github.com/BalogunEmmanuelKehinde) • [LinkedIn](https://linkedin.com/in/emmanuel-balogun-kehinde)
